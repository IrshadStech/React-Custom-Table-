...
const GetPaginationButtons = (props) => {
  ...
  return childs.map((item) => (
    <button
      key={item.key}
      className={`pagination-btn ${item.value == "..." ? "transparent " : ""} ${
        item.value == props.currentActiveButton + 1 ? "active" : ""
      }`}
    ...
    return (
      <div className="table-wrapper">
        <div className="table-header">
          ...
          <button className="theader-btn">
            <CSVLink
              data={this.state.listItemToPrint}
              headers={listItemToDisplay.map((item) => item.name)}
